...
public class FormatUtil {
	...
	/*
	 * public static void deepCopyPropertiesIgnoreNull(Object target, Object
	 * resource) { PropertyUtilsBean propertyUtils =
	 * BeanUtilsBean.getInstance().getPropertyUtils(); PropertyDescriptor[]
	 * srcDescriptors = propertyUtils.getPropertyDescriptors(resource); for
	 * (PropertyDescriptor origDescriptor : srcDescriptors) { String name =
	 * origDescriptor.getName(); if ("class".equals(name)) { continue; // No
	 * point in trying to set an object's class } if
	 * (propertyUtils.isReadable(resource, name) &&
	 * propertyUtils.isWriteable(target, name)) { try { Object value =
	 * propertyUtils.getSimpleProperty(resource, name); if(null == value)
	 * continue; //空值不复制 if(deepClone(value) == null){ continue; }
	 * propertyUtils.setProperty(target, name, deepClone(value)); } catch
	...
	public static void main(String[] args) {
	 String s = FormatUtil.getPercent(5,7);
