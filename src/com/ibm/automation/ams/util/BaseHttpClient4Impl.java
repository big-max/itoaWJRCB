...
public class BaseHttpClient4Impl {
	...
	public JsonNode query(final JsonNode req) {
		...
		String payload = req.has("payload") ? req.get("payload").toString() : null;
		// System.out.println("p:" + req.get("payload"));

		String charset = req.has("charset") ? req.get("charset").textValue() : "utf-8";
		;

		JsonNode headers = req.has("headers") ? req.get("headers") : om.createObjectNode();

		String mimeType = headers.has("Content-Type") ? headers.get("Content-Type").asText() : "application/json";
		...
		try {
			...
			if (null != payload) {
				...
				HttpEntity he = new StringEntity(payload, ContentType.create(mimeType, Charset.forName(charset)));

				HttpEntityEnclosingRequestBase hrb = new HttpEntityEnclosingRequestBase() {

					public String getMethod() {
						return req.has("method") ? req.get("method").asText() : "GET";
					}
		...
		try {
			...
			for (Header header : chr.getAllHeaders()) {
				...
				String headerVal = respHeaders.has(headerKey) ? respHeaders.get(headerKey).asText() : null;
				if (null == headerVal) {
					respHeaders.put(headerKey, header.getValue());
				} else {
					respHeaders.put(headerKey, headerVal + ";" + header.getValue());
				}
			}
