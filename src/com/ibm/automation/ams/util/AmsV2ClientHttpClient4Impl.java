...
public class AmsV2ClientHttpClient4Impl extends BaseHttpClient4Impl implements AmsClient {
	...
	public AmsV2ClientHttpClient4Impl(String str) {
		...
		String endPoint = amsCfg.getProperty(str);
	...
	public String getUrl(String... args) {
		...
		if (args.length == 1) {
			...
			if (attrKey != null && attrVal != null) {
				url = String.format("%s/%s/%s/%s", endpoint, table, attrKey, attrVal);
	...
	public <T> T auth(Class<T> clazz, JsonNode credential) {
		...
		try {
			...
			if (null != resp && resp.has("statusCode") && resp.get("statusCode").intValue() == 200) {
	...
	public ObjectNode list_one(ArrayNode sort, JsonNode query, String... args) {
		...
		resp = (ObjectNode) this.query(req);
		return (ObjectNode) resp.get("payload");
	}
	
	public String list_get(ArrayNode sort, JsonNode query, String... args) {
		String url = getUrl(args);
		ObjectNode resp = null;
		...
		resp = (ObjectNode) this.query(req);
		return  resp.get("payload").toString();
	}
