...
public class MQCLUSTERController {
	...
	private static Logger logger = Logger.getLogger(MQCLUSTERController.class);
	...
	/**
	 * @see 参数全部准备齐全，开始组装json字符串
	 * 
	 */
	@RequestMapping("/installMqClusterInfo.do")
	...
	//	ArrayNode all_complete_savesArrNode = om.createArrayNode();
		...
		try {
			...
			// e.printStackTrace();
			logger.debug("installMqClusterInfo::" + e.getMessage());
	...
	//		outNode.putPOJO("password_list", password_listArrNode);
			if ("yes".equalsIgnoreCase(temp.getCompletesave())) {
				ObjectNode fullRep = om.createObjectNode();
				fullRep.put(temp.getIp(), temp.getQmgrname());
				all_complete_savesArrNode.put(temp.getIp(), temp.getQmgrname());
				final_o.put("role", 1);
			} else {
				final_o.put("role", 0);
			}
		...
		logger.info("installMqClusterInfo.do::" + outNode.toString());
		...
		// 唯一索引
		final_on.put("playbook-name", "mqcluster");// task-name task 名
		final_on.put("product-name", "mq");// product-name 产品名
		final_on.put("param-content", EncodeUtil.encode(outNode.toString()));// base64
																				// //
																				// 编码yam
																				// //
																				// //
		...
		logger.info("installMqClusterInfo::" + final_on.toString());
		String response = "";
		try {
			...
			if (ons.get("status").asText().equalsIgnoreCase("ok")) {
				logger.info("installMqClusterInfo.do::正在跳转到getLogInfo.do");
				return "redirect:/getLogInfo.do";
			}
		} catch (NetWorkException | IOException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
			logger.error("installMqClusterInfo.do::" + e.getMessage());
	...
	@RequestMapping("/toMqClusterNextPage.do")
	public String toMqClusterNextPage(HttpServletRequest request, HttpSession session) throws Exception {
		...
		String[] all_complete_saves = request.getParameterValues("all_complete_saves");// 所有是否完全存储库
		...
		request.setAttribute("mq_version", request.getParameter("mqversion"));//版本8.0 mq_version//介质名
		...
		if (status.equals("confirm")) {
			return "instance_mqcluster_comfirm";
		}
		return null;
	}
		...
		if (curPlaybook != null && curPlaybook.get("options") != null) {
			try {
				...
				if (type.equalsIgnoreCase("aix")) {
					...
					request.setAttribute("aix_data", options.get("os_para_ulimit").get("aix_data").asText());
					request.setAttribute("aix_stack", options.get("os_para_ulimit").get("aix_stack").asText());
				}

			} catch (IOException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
				logger.error("getmqclusterLogInfoDetail::"+e.getMessage());
			}
		}
